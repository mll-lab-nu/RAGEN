...
run_experiment() {
    ...
    common_overrides+=("actor_rollout_ref.rollout.rollout_filter_include_zero=${include_zero}")
    ...
    local name="${task}-${algo}-${filter}-${model_name}-minibs16"
    ...
    CUDA_VISIBLE_DEVICES="${gpu_list}" python train.py --config-name "$config" \
        ...
        trainer.project_name="ragen_rebuttal_larger_models" \
