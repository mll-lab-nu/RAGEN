#!/bin/bash
# Grouping Timing Ablation: Different env_groups × group_size layouts × filter/no-filter
# Task: Sokoban only | Model: Qwen2.5-3B | Algorithm: PPO
# Group layouts: 8x16, 128x1, 64x2, 32x4, 16x8 — all total 128 samples
# Filtering rule: filter => top_p=0.9 (linear), nofilter => top_p=1.0
# Special: 10 steps only, no eval (test_freq=50), no MI computation
...
GROUP_LAYOUTS=("128:1" "8:16" "64:2" "32:4" "16:8")
...
usage() {
    ...
    echo "Options:"
    echo "  --steps N             Training steps (default: 10)"
...
LOG_FILE="logs/grouping_timing_${MODEL_NAME}.log"
RESULT_ROOT="logs"
CHECKPOINT_ROOT="model_saving/grouping_timing_${MODEL_NAME}"

mkdir -p logs
mkdir -p "$RESULT_ROOT"
mkdir -p "$CHECKPOINT_ROOT"

echo "=== Grouping Timing Runner: $(date) ===" | tee "$LOG_FILE"
...
echo "MI disabled | val_before_train=False | test_freq=50 (no eval in ${STEPS} steps)" | tee -a "$LOG_FILE"
...
run_experiment() {
    ...
    local name="${TASK}-PPO-${filter}-grp${layout_label}-${MODEL_NAME}-timing"
    local task_dir="${RESULT_ROOT}/grouping_timing_${TASK}_${MODEL_NAME}"
    ...
    CUDA_VISIBLE_DEVICES="${gpu_list}" python train.py --config-name "$CONFIG" \
        ...
        trainer.project_name="ragen_rebuttal_grouping_timing" \
...
for layout in "${SELECTED_GROUP_LAYOUTS[@]}"; do
    ...
    for filter in "${SELECTED_FILTERS[@]}"; do
        # 128x1: group_size=1, filter has no effect — only run nofilter
        if [ "$group_size" -eq 1 ] && [ "$filter" = "filter" ]; then
            continue
        fi
...
QUEUE_FILE=$(mktemp -t ragen_grouping_timing_queue.XXXXXX)
...

{
    ...
    echo "GPU per exp: ${GPUS_PER_EXP}x${GPU_MODEL_LABEL} | Model: ${MODEL_NAME} | Task: ${TASK} | Algo: PPO | Steps: ${STEPS}"
    for group_label in "${GROUP_LABELS[@]}"; do
        ...
        for exp in "${EXPERIMENTS[@]}"; do
            ...
            name="${TASK}-PPO-${filter}-grp${layout_label}-${MODEL_NAME}-timing"
            task_dir="${RESULT_ROOT}/grouping_timing_${TASK}_${MODEL_NAME}"
