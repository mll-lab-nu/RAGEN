#!/bin/bash
# Rollout filtering runs across algorithms (PPO/DAPO/GRPO/DrGRPO) and tasks.
# Model: Qwen2.5-3B only
# Supported filters:
#   - nofilter     => top_p=1.0, include_zero=True
#   - linear_top_p_09 => top_p=0.9, top_p_prob_mode=linear, include_zero=False, selection_eps=0.01
#   - top_k_025    => top_k=0.25, largest, include_zero=True
# All three filters use rollout_filter_metric=reward_variance.
...
FILTER_MODES=("nofilter" "linear_top_p_09" "top_k_025")
FILTERS_OPTION="all"
...
usage() {
    ...
    echo "  --filters LIST        Comma-separated filter modes (nofilter,linear_top_p_09,top_k_025,all). Default: all"
...
LOG_FILE="logs/different_rv_filter_${MODEL_NAME}.log"
RESULT_ROOT="logs"
CHECKPOINT_ROOT="model_saving/different_rv_filter_${MODEL_NAME}"

mkdir -p logs
mkdir -p "$RESULT_ROOT"
mkdir -p "$CHECKPOINT_ROOT"

echo "=== Reward Variance Filter Runner for ${MODEL_NAME}: $(date) ===" | tee "$LOG_FILE"
...
run_experiment() {
    ...
    local filter_metric="reward_variance"
    local filter_top_p_prob_mode="linear"
    local filter_type="largest"
    local filter_include_zero="True"
    local filter_selection_eps="0.01"
    case "$filter" in
        nofilter)
            filter_value=1.0
    ...
    local task_dir="${RESULT_ROOT}/different_rv_filter_${task}_${MODEL_NAME}"
    ...
    CUDA_VISIBLE_DEVICES="${gpu_list}" python train.py --config-name "$config" \
        ...
        trainer.project_name="ragen_release_different_rv_filter" \
...
PY
    ...
    local summary_line="task=${task} | algo=${algo} | filter=${filter} | model=${MODEL_NAME} | steps=${STEPS} | filter=${filter_strategy}:${filter_value}:${filter_metric}:include_zero=${filter_include_zero}:type=${filter_type}:prob_mode=${filter_top_p_prob_mode} | ppo_micro=${PPO_MICRO_BATCH_SIZE_PER_GPU} | log_prob_micro=${LOG_PROB_MICRO_BATCH_SIZE_PER_GPU} | train_time=${TRAIN_TIME}s | eval_time=${EVAL_TIME}s | total_time=${TOTAL_TIME_METRIC}s | wall_time=${TOTAL_TIME}s | gpu=${gpu_label} | status=${status}"
    ...
    for candidate in "${candidates[@]}"; do
        ...
        case "$candidate" in
            nofilter|no_filter|none)
                SELECTED_FILTERS+=("nofilter")
                ;;
            linear_top_p_09|linear_top_p|linear|linear_top_p_filter|lineartoppfilter)
                SELECTED_FILTERS+=("linear_top_p_09")
                ;;
            top_k_025|top_k_0.25|topk025|topk0.25|topk_025|topk025filter)
                SELECTED_FILTERS+=("top_k_025")
                ;;
            "")
                continue
                ;;
...
QUEUE_FILE=$(mktemp -t ragen_different_rv_filter_queue.XXXXXX)
...

{
    ...
    for group_label in "${GROUP_LABELS[@]}"; do
        ...
        for exp in "${EXPERIMENTS[@]}"; do
            ...
            task_dir="${RESULT_ROOT}/different_rv_filter_${task}_${MODEL_NAME}"
