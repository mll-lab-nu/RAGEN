...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # Check if checkpoint was saved with a different world size
        expected_model_path = os.path.join(local_path, f"model_world_size_{self.world_size}_rank_{self.rank}.pt")
        fallback_model_path = os.path.join(local_path, "model_world_size_1_rank_0.pt")
        use_resharding = (
            self.should_load_model
            and not os.path.exists(expected_model_path)
            and os.path.exists(fallback_model_path)
            and self.world_size > 1
        )

        if use_resharding:
            log_with_rank(
                f"Checkpoint world_size mismatch: expected world_size={self.world_size} but found world_size=1. "
                f"Loading full state dict and resharding across {self.world_size} ranks.",
                rank=self.rank, logger=logger,
            )
            # Load full state dict from 1-GPU checkpoint, then let FSDP reshard
            from torch.distributed.fsdp import FullStateDictConfig
            full_cfg = FullStateDictConfig(offload_to_cpu=True, rank0_only=False)
            with get_fsdp_state_ctx(self.model, StateDictType.FULL_STATE_DICT, full_cfg, None):
                local_model_path = copy_to_local(fallback_model_path)
                model_state_dict = torch.load(local_model_path, weights_only=False)
                self.model.load_state_dict(model_state_dict)
                log_with_rank(f"Loaded and resharded model from {fallback_model_path}", rank=self.rank, logger=logger)
        else:
        ...
        if self.should_load_extra:
            ...
            # Fallback to world_size=1 extra state if current world_size file doesn't exist
            if not os.path.exists(remote_extra_state_path):
                fallback_extra_path = os.path.join(local_path, "extra_state_world_size_1_rank_0.pt")
                if os.path.exists(fallback_extra_path):
                    log_with_rank(f"Extra state world_size mismatch, falling back to {fallback_extra_path}", rank=self.rank, logger=logger)
                    remote_extra_state_path = fallback_extra_path
                else:
                    log_with_rank(f"No extra state found at {remote_extra_state_path}, skipping extra state loading", rank=self.rank, logger=logger)
                    remote_extra_state_path = None

            if remote_extra_state_path is not None:
